...
| `/review-pr` | Review pull requests with quality, security, and best practices analysis |
...
This project includes specialized Claude Code agents in `.claude/agents/` that are automatically selected for specific tasks:

| Agent | When it's used |
|-------|----------------|
| `coder` | Implementing new features, writing or refactoring code |
| `code-review` | Reviewing code quality, detecting vulnerabilities, checking standards |
| `deep-dive` | Deep architectural analysis, complex bug investigation, research |
| `better-auth-expert` | Any changes to authentication (auth.ts, auth-client.ts, auth components) |
| `polar-payments-expert` | Polar payment integration (webhooks, checkout, subscriptions) |
| `game-developer` | Game systems, graphics optimization, multiplayer networking |

---

## 🎨 Skills

This project includes the `frontend-design` skill in `.claude/skills/`:

| Skill | Description |
|-------|-------------|
| `frontend-design` | Creates distinctive, production-grade frontend interfaces with high design quality. Avoids generic AI aesthetics. Use when building pages, components, or any web UI. |

Invoke it naturally by asking Claude to build or style a UI component or page.

---

## 🔌 MCP Servers

This project is configured with MCP (Model Context Protocol) servers in `.mcp.json`:

| Server | Purpose |
|--------|---------|
| `shadcn` | Add and manage shadcn/ui components directly via Claude (`npx shadcn@latest mcp`) |
| `playwright` | Browser automation and end-to-end testing (`npx @playwright/mcp@latest`) |
| `next-devtools` | Next.js dev server diagnostics, route inspection, and error detection (`npx next-devtools-mcp@latest`) |

These are automatically available when using Claude Code in this project.

> **Note:** The `.claude/agents/coder.md` file contains project context that references a different project ("autocoder" with Python/neobrutalism). If you use the `coder` agent, update its project context section to match this Next.js 16 project.
